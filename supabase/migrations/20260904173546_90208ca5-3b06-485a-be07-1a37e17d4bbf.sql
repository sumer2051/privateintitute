...
ALTER TABLE public.transactions ADD CONSTRAINT transactions_status_check CHECK (status = ANY (ARRAY['pending','processing','under_review','compliance_hold','reviewed','clearing','completed','posting','failed','cancelled']));
...
BEGIN
  ...
  END IF;
  IF p_status NOT IN ('pending','processing','under_review','compliance_hold','reviewed','clearing','completed','posting','failed','cancelled') THEN
